...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        ).toArray().then(function(categories) {
     // console.log(categories); // Use this to debug
      var soma = 0;
     
     for (var i = 0, len = categories.length; i < len; i++) {
        soma += categories[i].num;
        }
      category = {
            _id: "All",
            num: soma
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
       ...
       //console.log(page);
        ...
        pageItems = this.db.collection("item").find({category: category}).sort({_id: 1}).skip(page > 0 ? ((page)*itemsPerPage) : 0).limit(itemsPerPage).toArray().then(function(itens) {
            ...
            pageItems = this.db.collection("item").find().sort({_id: 1}).skip(page > 0 ? ((page)*itemsPerPage) : 0).limit(itemsPerPage).toArray().then(function(itens) {
               // console.log(itens);
                callback(itens);
            });
        }
    ...
    this.getNumItems = function(category, callback) {
     ...
     for (var i = 0, len = categories.length; i < len; i++) {
        ...
        var numItems = this.db.collection("item").aggregate([
                {
                    $match: {
                        category: category
        ...
        ).toArray().then(function(number){
          // console.log(number);
            callback(number[0].num);
        });
        /*
         * TODO-lab1C:
         *
